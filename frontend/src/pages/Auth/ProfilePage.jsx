    ...
    if (!menteeData) {
      return (
        <div className="text-center py-16 glass-card rounded-2xl bg-gradient-to-b from-white/90 to-white/70 shadow-xl">
          ...
          <p className="text-gray-600 max-w-md mx-auto mb-8">No mentee profile found. Let's create your professional profile to showcase your skills and achievements.</p>
          <motion.button 
      ...
      <div className="space-y-8">
        ...
        <section className="glass-card p-8 rounded-2xl bg-gradient-to-b from-white/90 to-white/70 shadow-lg transition-all hover:shadow-xl">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            >
              <h3 className="font-semibold text-gray-700 mb-3 text-lg">Class 10</h3>
              <p className="text-gray-900 font-medium">{menteeData?.academics?.class10?.school || 'Not specified'}</p>
              <p className="text-gray-600">Percentage: <span className="font-medium">{menteeData?.academics?.class10?.percentage || 'N/A'}%</span></p>
              <p className="text-gray-600">Year: <span className="font-medium">{menteeData?.academics?.class10?.yearOfCompletion || 'N/A'}</span></p>
            </motion.div>
            ...
            <motion.div 
              ...
              className="bg-white/60 p-6 rounded-xl shadow-sm hover:shadow-md transition-all"
            >
              <h3 className="font-semibold text-gray-700 mb-3 text-lg">Class 12</h3>
              <p className="text-gray-900 font-medium">{menteeData?.academics?.class12?.school || 'Not specified'}</p>
              <p className="text-gray-600">Percentage: <span className="font-medium">{menteeData?.academics?.class12?.percentage || 'N/A'}%</span></p>
              <p className="text-gray-600">Year: <span className="font-medium">{menteeData?.academics?.class12?.yearOfCompletion || 'N/A'}</span></p>
            </motion.div>
            ...
            >
              <h3 className="font-semibold text-gray-800 mb-3 text-lg">Current Education</h3>
              <p className="text-gray-900 font-medium text-lg">{menteeData?.academics?.currentEducation?.institution || 'Not specified'}</p>
              <p className="text-gray-700">
                {menteeData?.academics?.currentEducation?.course || 'N/A'} 
                {menteeData?.academics?.currentEducation?.specialization && 
                  <span> - {menteeData?.academics?.currentEducation?.specialization}</span>
                }
              </p>
              <div className="flex flex-wrap gap-6 mt-2">
                <p className="text-gray-600">
                  <span className="font-medium">Year:</span> {menteeData?.academics?.currentEducation?.yearOfStudy || 'N/A'}
                </p>
                <p className="text-gray-600">
                  <span className="font-medium">CGPA:</span> {menteeData?.academics?.currentEducation?.cgpa || 'N/A'}
                </p>
              </div>
            </motion.div>
          </div>
        ...
        <section className="glass-card p-8 rounded-2xl bg-gradient-to-b from-white/90 to-white/70 shadow-lg transition-all hover:shadow-xl">
          ...
          <div className="space-y-8">
              ...
              <div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  {menteeData.extracurricular.map((activity, index) => (
                    ...
                    >
                      <h4 className="font-medium text-lg text-gray-800">{activity?.activity || 'Activity'}</h4>
                      <p className="text-gray-700 font-medium">{activity?.role || 'Role'}</p>
                      <p className="text-sm text-gray-500 mt-1">{activity?.duration || 'Duration'}</p>
                      <p className="text-sm text-gray-600 mt-3 line-clamp-3">{activity?.description || 'No description provided'}</p>
                    </motion.div>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  {menteeData.internships.map((internship, index) => (
                    ...
                    >
                      <h4 className="font-medium text-lg text-indigo-900">{internship.company || 'Company'}</h4>
                      <p className="text-indigo-700 font-medium">{internship.role || 'Role'}</p>
                      <p className="text-sm text-indigo-500 mt-1">{internship.duration || 'Duration'}</p>
                      <p className="text-sm text-gray-600 mt-3 line-clamp-3">{internship.description || 'No description provided'}</p>
                    </motion.div>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  {menteeData.achievements.map((achievement, index) => (
                    ...
                    >
                      <h4 className="font-medium text-lg text-amber-900">{achievement.title || 'Achievement'}</h4>
                      <p className="text-sm text-amber-600 mt-1">Year: {achievement.year || 'N/A'}</p>
                      <p className="text-sm text-gray-600 mt-3">{achievement.description || 'No description provided'}</p>
                    </motion.div>
                  ))}
                </div>
              </div>
            ...
            {!menteeData.extracurricular?.length && !menteeData.internships?.length && !menteeData.achievements?.length && (
        ...
        <section className="glass-card p-8 rounded-2xl bg-gradient-to-b from-white/90 to-white/70 shadow-lg transition-all hover:shadow-xl">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            >
              <h3 className="font-semibold text-gray-800 mb-3 text-lg">Short Term Goals</h3>
              <p className="text-gray-700 italic">{menteeData?.futureGoals?.shortTerm || 'No short-term goals specified'}</p>
            </motion.div>
            ...
            >
              <h3 className="font-semibold text-gray-800 mb-3 text-lg">Long Term Goals</h3>
              <p className="text-gray-700 italic">{menteeData?.futureGoals?.longTerm || 'No long-term goals specified'}</p>
            </motion.div>
            ...
            {menteeData?.futureGoals?.dreamCompanies?.length > 0 && (
              ...
              >
                <h3 className="font-semibold text-gray-800 mb-3 text-lg">Dream Companies</h3>
                <div className="flex flex-wrap gap-2">
                  {menteeData.futureGoals.dreamCompanies.map((company, index) => (
                    <motion.span 
                      key={index} 
      ...
      <div className="relative">
        ...
        <div className="relative -mt-16 ml-8 flex items-end">
          ...
          <div className="ml-6 mb-4">
            <h1 className="text-4xl font-bold leading-9 text-white drop-shadow-md">{user.name}</h1>
