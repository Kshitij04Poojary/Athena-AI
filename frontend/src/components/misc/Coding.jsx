...
const CodeEditor = () => {
  ...
  return (
    <div className={`mt-15 mx-3 sm:mx-5 mb-5 flex flex-col min-h-screen rounded-lg overflow-y-auto shadow-xl ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-white text-gray-800'}`}>
      {/* Header */}
      <div className={`flex flex-col sm:flex-row justify-between items-center p-3 sm:p-4 gap-2 ${theme === 'dark' ? 'bg-gray-800' : 'bg-gray-100'}`}>
        <div className="flex items-center space-x-2">
          <h2 className="text-base sm:text-lg font-semibold">Code Playground</h2>
        </div>

        <div className="flex flex-wrap items-center justify-between sm:justify-end gap-2 w-full md:w-auto">
          {/* Language selector */}
          <div className="relative group">
            <div className={`flex items-center px-3 py-2 rounded-md cursor-pointer w-28 sm:w-32 ${theme === 'dark' ? 'bg-gray-700 hover:bg-gray-600' : 'bg-white hover:bg-gray-50 border border-gray-300'}`}>
              <div className={`w-3 h-3 rounded-full mr-2 ${getLanguageColor(language)}`}></div>
              <span className="text-xs sm:text-sm font-medium flex-1 truncate">{language.charAt(0).toUpperCase() + language.slice(1)}</span>
            ...
            {/* Dropdown menu */}
            <div className={`absolute z-10 mt-1 w-28 sm:w-32 rounded-md shadow-lg ${theme === 'dark' ? 'bg-gray-700' : 'bg-white border border-gray-200'} hidden group-hover:block`}>
              <div className="py-1">
                {['javascript', 'python', 'java', 'cpp'].map((lang) => (
                  <button
                    key={lang}
                    onClick={() => setLanguage(lang)}
                    className={`w-full text-left px-4 py-2 text-xs sm:text-sm flex items-center ${language === lang ? (theme === 'dark' ? 'bg-gray-600' : 'bg-gray-100') : ''} ${theme === 'dark' ? 'hover:bg-gray-600' : 'hover:bg-gray-50'}`}
                  >
          ...
          <div className="flex gap-2">
            <button
              onClick={runCode}
              disabled={isRunning}
              className={`px-2 sm:px-4 py-2 rounded-md font-medium flex items-center space-x-1 transition transform hover:-translate-y-0.5 text-xs sm:text-base ${isRunning ? 'bg-gray-500 cursor-not-allowed' : 'bg-green-500 hover:bg-green-400 text-white'}`}
            >
              <span>{isRunning ? '⏳' : ' '}</span>
            ...
            <button
              ...
              className={`px-2 sm:px-4 py-2 rounded-md font-medium flex items-center space-x-1 transition transform hover:-translate-y-0.5 text-xs sm:text-base ${isSubmitting ? 'bg-gray-500 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-400 text-white'}`}
            >
              <span>{isSubmitting ? '⏳' : ' '}</span>
              <span className="hidden sm:inline">{isSubmitting ? 'Submitting...' : 'Submit Code'}</span>
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
        ...
        <div className={`flex flex-col flex-1 ${theme === 'dark' ? 'border-r border-gray-700' : 'border-r border-gray-200'} h-1/2 md:h-auto`}>
          <div className={`flex justify-between items-center px-3 sm:px-4 py-2 ${theme === 'dark' ? 'bg-gray-800 border-b border-gray-700' : 'bg-gray-50 border-b border-gray-200'}`}>
            <h3 className={`text-xs sm:text-sm font-semibold ${theme === 'dark' ? 'text-purple-400' : 'text-purple-600'}`}>Code</h3>
          ...
          <div className="flex flex-1 overflow-hidden">
            {/* Line numbers */}
            <div className={`py-3 px-2 text-right select-none ${theme === 'dark' ? 'bg-gray-800 text-gray-500' : 'bg-gray-50 text-gray-400'} font-mono text-xs sm:text-sm w-6 sm:w-12 overflow-y-hidden`}>
              {code.split('\n').map((_, idx) => (
        ...
        <div className="flex flex-col flex-1 h-1/2 md:h-auto">
          ...
          <div className={`flex ${theme === 'dark' ? 'bg-gray-800 border-b border-gray-700' : 'bg-gray-50 border-b border-gray-200'}`}>
            <button
              ...
              className={`px-2 sm:px-4 py-2 text-xs sm:text-sm font-medium border-b-2 ${activeTab === 'output'
            ...
            <button
              ...
              className={`px-2 sm:px-4 py-2 text-xs sm:text-sm font-medium border-b-2 flex items-center ${activeTab === 'error'
          ...
          {/* Output content */}
          {activeTab === 'output' && (
            <pre className={`flex-1 p-2 sm:p-4 font-mono text-xs sm:text-sm leading-6 overflow-auto whitespace-pre-wrap ${theme === 'dark' ? 'bg-gray-900 text-gray-100' : 'bg-white text-gray-800'}`}>
              {output || 'Run your code to see output here...'}
            </pre>
          )}
          {/* Error content */}
          {activeTab === 'error' && (
            <div className={`flex-1 p-2 sm:p-4 overflow-auto ${theme === 'dark' ? 'bg-gray-900' : 'bg-white'}`}>
              {hasErrors ? (
